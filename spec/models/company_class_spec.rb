...
describe CompanyClass do
  ...
  context "(Valid)" do
  	...
  	it "when start_time is the same as AgmaProfile rehearsal_start" do
  		@cclass.start_time = profile.rehearsal_start_time
  		@cclass.end_time = min_to_formatted_time(profile.rehearsal_start_min + profile.rehearsal_increment_min)
  		should be_valid
  	end
  	
  	it "when end_time is the same as AgmaProfile rehearsal_end" do
  		@cclass.start_time = min_to_formatted_time(profile.rehearsal_end_min - profile.rehearsal_increment_min)
  		@cclass.end_time = profile.rehearsal_end_time
  		should be_valid
  	end
  ...
  context "(Invalid)" do  	
  	it "when duration is NOT multiple of AgmaProfile rehearsal_increment_min" do
  		@cclass.start_time = profile.rehearsal_start_time
  		@cclass.end_time = min_to_formatted_time(profile.rehearsal_start_min + profile.rehearsal_increment_min - 5)
  		should_not be_valid
  	end
  	
  	it "when start_time is before AgmaProfile rehearsal_start" do
  		@cclass.start_time = min_to_formatted_time(profile.rehearsal_start_min - 5)
  		@cclass.end_time = min_to_formatted_time(profile.rehearsal_start_min - 5 + profile.rehearsal_increment_min)
  		should_not be_valid
  	end
  	
  	it "when end_time is after AgmaProfile rehearsal_end" do
  		@cclass.start_time = min_to_formatted_time(profile.rehearsal_end_min + 5)
  		@cclass.end_time = min_to_formatted_time(profile.rehearsal_end_min + 5 + profile.rehearsal_increment_min)
  		should_not be_valid
  	end
  end
