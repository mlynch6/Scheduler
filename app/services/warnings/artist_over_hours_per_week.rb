module Warnings
	  ...
	  def artists
		...
		def messages
			...
			artists.each do |artist|
				overage = artist.worked_min.to_i - max_hours_per_week*60
				out << "#{artist.full_name} is over the maximum rehearsal allowance of #{max_hours_per_week} hrs/week by #{min_to_words(overage)}"
			end
	...
	private
		def get_over_rehearsed_artists
			if contract && max_hours_per_week.present?
				max_min_per_week = max_hours_per_week * 60
				
				people = Person.agma_members.joins(:events).scoped
				events = Event.where(schedulable_type: 'Rehearsal').for_week(start_date).reorder(nil).scoped
				people = people.merge(events)
				@overworked_artists = people.select("people.*, sum(DATE_PART('day', events.end_at-events.start_at)*24 + DATE_PART('hour', events.end_at-events.start_at)*60 + DATE_PART('minute', events.end_at-events.start_at)) as worked_min").group("people.id").having("sum(DATE_PART('day', events.end_at-events.start_at)*24 + DATE_PART('hour', events.end_at-events.start_at)*60 + DATE_PART('minute', events.end_at-events.start_at)) > :max_min_per_week", max_min_per_week: max_min_per_week)
			end
		end
